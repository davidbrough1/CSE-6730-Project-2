...
def intFaceNodes(intx, inty, intz, setA, setC, setE, Row_ELM_f, Layer_ELM_f, Block_ELM_f):
    ...
    n2plus_n3plus = set(NodeYp).intersection(set(NodeZp)).difference(set(NodeXp), set(NodeXn))
...
def generateAbaqusInp(inputFileName, ms, viscoelastic=False):
    ...
    if (viscoelastic):
        ...
        f.writelines(('0.0769, 0.0769, 0.75', nl))
    else:
    ...
    if (viscoelastic):
        ...
        f.writelines(('*Boundary, Amplitude=disp1', nl))
        f.writelines(('**', nl))
        ...
        f.writelines(('{},1,1,0.002'.format(setE), nl))
        f.writelines(('{},1,1,0.002'.format(setF), nl))
        f.writelines(('{},1,1,0.002'.format(setG), nl))
        f.writelines(('{},1,1,0.002'.format(setH), nl))
        ...
        f.writelines(('0.5,10,1e-05,0.5', nl))
        f.writelines(('** Name: BC-1 Type: Displacement/Rotation', nl))
        f.writelines(('*Boundary, Amplitude=disp2', nl))
        f.writelines(('**', nl))
        f.writelines(('{},1,3,0'.format(setA), nl))
        f.writelines(('{},1,3,0'.format(setB), nl))
        f.writelines(('{},1,3,0'.format(setC), nl))
